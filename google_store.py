...
class PanoFetcher:
    ...
    def store_pano(store_dir, pano_basic, img):
        with open(store_dir + '.json', 'w') as outfile:
            ...
            outfile.close()
        scipy.misc.imsave(store_dir + '.jpg', img)
...
def get_panorama_metadata(panoid=None, lat=None, lon=None, radius=30):
    """
    :param panoid: a specific Id google give to each panoramo
    :param lat:
    :param lon:
    :param radius:
    :return: find the nearest pano according to panoId or lat/lon in certain radius
    """
...
def get_panorama(panoid, zoom):
    # TODO: h_base, w_base should be renamed...
    h = pow(2, zoom - 1)
    w = pow(2, zoom)
    h_base = 416
    w_base = 512
...
def get_panorama_tile(panoid, zoom, x, y):
    """:param panoid:  value from the panorama metadata
    :param zoom: zoom range is 0->NumZoomLevels inclusively
    :param x:
    :param y:
    :return:
    """
    ...
    url += '&v=4'  # version
    url %= (base_uri, panoid, zoom, x, y)
    return url_to_image_python3(url)
